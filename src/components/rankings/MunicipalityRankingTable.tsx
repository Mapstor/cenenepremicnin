      ...
      <div className="space-y-3">
        {data.map((item) => (
          <Link
            ...
            className="block bg-white border border-gray-200 rounded-lg p-3 sm:p-4 hover:border-emerald-300 hover:shadow-sm transition-all"
          >
            <div className="flex items-start sm:items-center gap-3 sm:gap-4">
              ...
              {/* Municipality name and price - stacked on mobile */}
              <div className="flex-1 min-w-0 overflow-hidden">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs sm:text-sm text-gray-500 mt-0.5 ml-5 sm:ml-6">
                      {item.steviloTransakcij} transakcij
                    </div>
                  </div>

                  {/* Price and trend */}
                  <div className="sm:text-right flex-shrink-0 ml-5 sm:ml-0 mt-1 sm:mt-0">
                    <div className="font-bold text-gray-900 text-sm sm:text-base">
                      {formatPricePerM2(item.medianaCenaM2)}
                    </div>
                    {item.povprecjeCenaM2 && (
                      <div className="text-xs text-gray-500">
                        povp. {formatPricePerM2(item.povprecjeCenaM2)}
                      </div>
                    )}
                    {showTrend && item.trendYoY !== undefined && (
                      <div
                        className={`flex items-center sm:justify-end gap-1 text-xs sm:text-sm ${
